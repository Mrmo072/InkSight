...
- Regeneration keeps the previous AI answer visible when a replacement request fails, dropped bubbles retain their new layout position, wheel input over a selected bubble scrolls its content without zooming the graph, graph selection/expansion survives project reloads, deliberate double-click expansion is less prone to accidental activation, and saving a project also updates the reload-recovery snapshot. Long project names no longer exceed the browser directory-picker ID limit.
