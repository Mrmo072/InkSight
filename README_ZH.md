...
![InkSight 阅读工作区](./docs/images/workspace-overview.png)

## 🖼️ 产品展示 (Product Tour)

### 阅读与标注保持在同一工作流

阅读原文的同时，可在标注面板中查看高亮、笔记、溯源状态以及加入思维导图等操作，无需在多个页面之间切换。

![InkSight 阅读与标注工作流](./docs/images/reading-annotations.png)

### 将原文片段发展为关联思路

可以从标注打开气泡图谱：单击选中气泡后再进行操作，双击展开完整内容，并通过父气泡中的高亮文字定位对应子气泡。气泡选中、展开状态、生成节点以及文字与子节点的关联都会随项目快照保存。

![InkSight 气泡图谱视图](./docs/images/graph-view.png)

### 在中文与英文之间切换

可在设置中切换应用语言与视觉主题，修改会立即生效并保存在当前设备上。

![InkSight 语言与外观设置](./docs/images/settings-language.png)
...
- **交互式气泡图谱**: 支持气泡选中、整体拖拽、双击展开、编辑、分支和重新生成，并在重新加载后保留父子节点间可点击的文字关联。
- **中英文界面**: 应用外壳与主要工作流可在简体中文和 English 之间即时切换。
