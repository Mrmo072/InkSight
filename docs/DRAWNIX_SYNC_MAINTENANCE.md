...
- 导出扩展名从 `.drawnix` 改成 `.inksight`
- 导出时附带 `bookMd5`、`bookName`、`bookId`
- 导出时附带 card/highlight 持久化数据
- 导入时恢复 InkSight 自己的 cards/highlights
- 导入时做书籍 MD5 匹配和重映射

判断：

- 这是典型的产品层能力，不属于 Drawnix 通用能力。
- 后续同步时应优先保住这一层，必要时甚至应该进一步外移到 InkSight adapter 层。
...
4. 评估将 `data/json.ts` 的 InkSight 定制外移
